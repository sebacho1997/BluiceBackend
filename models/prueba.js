...
router.get('/reporte-detalle/:conductorId', async (req, res) => {
  ...
  try {
    ...
    if (!pedidoIds.length) return res.status(404).send('No hay pedidos para este conductor hoy');
      ...
      content: [
        ...
        { text: 'Totales Generales', style: 'header', margin: [0, 10, 0, 5] },
        {
          table: {
            widths: ['*', 120],
            body: [
              ['Total Efectivo', `Bs${totalEfectivo.toString()}`],
              ['Total QR', `Bs${totalQr.toString()}`],
    ...
    pdfDoc.on('end', () => {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=reporte_horizontal_${conductorNombre}.pdf`);
      res.send(result);
    });
    pdfDoc.end();
    ...
    console.error(err);
    res.status(500).send('Error generando PDF horizontal');
  }
});
