...
router.get('/reporte-contratos/:conductorId/:anio/:mes', async (req, res) => {
  ...
  try {
    ...
    // Consumos dentro del mes especificado
    const consumosRes = await pool.query(`
      ...
      WHERE cc.contrato_id = ANY($1)
        AND EXTRACT(YEAR FROM cc.fecha) = $2
        AND EXTRACT(MONTH FROM cc.fecha) = $3
      ORDER BY cc.fecha
    `, [contratoIds, year, month]);
    ...
    const docDefinition = {
      ...
      content: [
        { text: `Reporte de Contratos y Consumos - ${year}-${month}`, style: 'header', alignment: 'center' },
    ...
    pdfDoc.on('end', () => {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=reporte_contratos_${conductorNombre}_${year}_${month}.pdf`);
      res.send(result);
    });
    pdfDoc.end();
    ...
    console.error(err);
    res.status(500).send('Error generando PDF de contratos por mes');
  }
});
