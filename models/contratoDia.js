...
router.get('/reporte-contratos/:conductorId', async (req, res) => {
  ...
  try {
    ...
    // Contratos del conductor asignados
    ...
    // Construcción del contenido PDF
    ...
    const docDefinition = {
      ...
      content: [
        { text: 'Reporte de Contratos y Consumos', style: 'header', alignment: 'center' },
    ...
    pdfDoc.on('end', () => {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=reporte_contratos_${conductorNombre}.pdf`);
      res.send(result);
    });
    pdfDoc.end();
    ...
    console.error(err);
    res.status(500).send('Error generando PDF de contratos');
  }
});
