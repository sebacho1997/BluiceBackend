...
router.get('/reporte-contratos-cliente-personalizado/:clienteId/:start/:end', async (req, res) => {
  const { clienteId, start, end } = req.params;
  ...
  try {
    ...
    const consumosRes = await pool.query(
      `SELECT cc.id AS consumo_id, cc.contrato_id, cc.monto_consumido, cc.fecha_entrega
       ...
       WHERE cc.contrato_id = ANY($1)
         AND cc.fecha_entrega::date BETWEEN $2 AND $3
       ORDER BY cc.contrato_id, cc.id`,
      [contratoIds, start, end]
    );
    ...
    const content = [
      ...
      buildDataTable(
        ['Contrato', 'Consumos', 'Unidades', 'Monto'],
    ...
    Object.entries(consumosPorContrato)
      ...
      .forEach(([contratoId, data]) => {
        content.push(sectionTitle(`Contrato #${contratoId}`));
    ...
    const docDefinition = buildDocDefinition({
      title: 'Reporte Personalizado de Contratos',
      subtitleLines: [
    ...
    pdfDoc.on('end', () => {
      ...
      res.setHeader(
        ...
        `attachment; filename=${buildReportFilename({
          ...
          reportType: 'contratos_personalizado',
    ...
    console.error(err);
    res.status(500).send('Error generando PDF personalizado de contratos');
  }
});
